...
class MyDataset(Dataset):
    def __init__(self, json_file, crop_size=(512, 512), top_crop=6, bottom_crop=250): 
        self.data = []
    ...
    def __getitem__(self, idx):
        ...
        seg_filename = source_filename.replace("persudo_optimized","seg")
        depth_filename = source_filename.replace("persudo_optimized","depth")
        infrared_filename = source_filename.replace("persudo_optimized","infrared")
        sate_filename = source_filename.replace("persudo_optimized","satellite")

        target_filename = source_filename.replace("persudo_optimized","persudo_perfect")
        mask_filename = source_filename.replace("persudo_optimized","persudo_mask").replace("jpg","npy")
        prompt = item['prompt']
        ...
        # sate_filename = source_filename.replace("source","satellite")
        ...
        def load_and_process_sate(filename):
            ...
            """
            # filename = f'/mnt/mydisk/fkx/CVPR/data/s1/{filename}'
            img = cv2.imread(filename)
            if img is None:
                raise FileNotFoundError(f"图像不存在：{filename}")
        ...
        def load_and_process_image(filename, unified_crop=None):
            ...
            if os.path.exists(filename):
                ...
                f_replace = filename.replace("persudo_perfect","persudo_optimized")
                img = cv2.imread(f_replace)
            ...
            return img_resized.astype(np.float32)  # 保持uint8类型，后续统一处理
        
        # temp_rgb = load_and_process_image(infrared_filename, unified_crop=None)  # 只做固定裁剪
        ...
        depth = load_and_process_image(depth_filename)
        infrared = load_and_process_image(infrared_filename)
        sate = load_and_process_sate(sate_filename)
        target = load_and_process_image(target_filename)
        mask = load_and_process_mask(mask_filename)
